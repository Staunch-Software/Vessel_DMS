...
export default function App() {
  ...
  const openBulkRestoreRecycleModal = useCallback(() => {
    if (recycleSelectIds.size === 0) return;
  ...
  const openBulkRestoreAllRecycleModal = useCallback(() => {
  ...
  return (
    <div className="dms-app-bg flex h-screen overflow-hidden">
              ...
              <div className="dms-top-chrome border-b border-slate-200 bg-white">
                <div className="dms-page-px py-3 flex items-center gap-2 flex-wrap">
                  ...
                  <div className="flex items-center gap-1 flex-wrap">
                    ...
                    {/* Restore selected */}
                    <button
                      onClick={openBulkRestoreRecycleModal}
                      disabled={recycleSelectIds.size === 0}
                      className="inline-flex items-center gap-1.5 rounded px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-100 border border-transparent hover:border-slate-200 transition disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                    ...
                    {/* Empty Recycle Bin */}
                    <button
                      onClick={openBulkDeleteAllRecycleModal}
                      disabled={deletedNodes.length === 0}
                      className="inline-flex items-center gap-1.5 rounded px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-100 border border-transparent hover:border-slate-200 transition disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                    ...
                    {/* Restore all items */}
                    <button
                      onClick={openBulkRestoreAllRecycleModal}
                      disabled={deletedNodes.length === 0}
                      className="inline-flex items-center gap-1.5 rounded px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-100 border border-transparent hover:border-slate-200 transition disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      <ArchiveRestore className="h-3.5 w-3.5 text-emerald-600" />
          ...
          <div className="relative w-full max-w-xl rounded-2xl bg-white shadow-2xl overflow-hidden">
            ...
            <div className="border-t border-slate-100 px-6 py-4 bg-slate-50 flex gap-3">
              ...
              >
                ...
                Permanent Delete {recycleSelectIds.size > 0 ? `(${recycleSelectIds.size})` : ""}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* ── Bulk Recycle Permanent Delete Confirmation Modal ────── */}
      {showBulkRestoreRecycleModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="absolute inset-0" onClick={() => setShowBulkRestoreRecycleModal(false)} />
          <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Selected items will be restored to their original location</p>
              </div>
            </div>
            <div className="mb-6 rounded-xl border border-emerald-100 bg-emerald-50/40 p-4 text-xs text-slate-600 leading-relaxed">
              You selected <strong className="text-emerald-700">{recycleSelectIds.size} item(s)</strong> to restore from the Recycle Bin.
            </div>
            <div className="flex gap-3">
              <button
                onClick={() => setShowBulkRestoreRecycleModal(false)}
                className="flex-1 rounded-lg border border-slate-200 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 transition cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={() => {
                  setShowBulkRestoreRecycleModal(false);
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm">
          ...
          <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-slate-800">Permanent Delete Selected Items?</h2>
